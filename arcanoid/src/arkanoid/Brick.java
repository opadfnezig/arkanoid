...
public class Brick
{
	...
	private GRect rect;
	public Brick(GPoint centerCoords) { rect = new GRect(centerCoords.getX(),centerCoords.getY(),width,height); }
	...
	{
		if(x >= rect.getX()&&(x<=rect.getX()+width)&&(y >= rect.getY())&&(y <= rect.getY()+height))
			return true;
		return false;
	}
